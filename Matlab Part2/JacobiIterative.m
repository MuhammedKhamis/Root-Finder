function[table,flagError] = JacobiIterative(A, b, x0, iterations, error)
time = tic;
flagError = 0;
try
    ...
    for i = 2 : iterations
        ...
        for k = 1 : num_of_variables
            if(k ~= j)
                res(1) = res(1) + A(j, k) * x(i - 1,k);
            end
        end
        x(i,j) = (b(j) - res(1))/A(j,j);
    ...
    printOutputFile('outputJacobiIterative.txt',table);
